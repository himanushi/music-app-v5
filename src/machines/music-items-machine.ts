...
import type { DocumentNode, ObservableQuery, WatchQueryFetchPolicy } from "@apollo/client";
      ...
      actions: {
        ...
        setFetchPolicy: assign({
          fetchPolicy: ({ variables }) => {
            if (variables?.conditions?.favorite || variables?.conditions?.isMine) {
              return "no-cache";
            }
        ...
        setParameters: assign({
          variables: ({ variables }, event) => {
            if ("params" in event) {
              return buildParameters<Mutable<ItemsQueryVariables>>(type, event.params, variables);
            }
